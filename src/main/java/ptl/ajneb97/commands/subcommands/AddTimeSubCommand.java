...
public class AddTimeSubCommand implements SubCommand {
    ...
    public void execute(CommandSender sender, String[] args, MessagesManager msgManager, FileConfiguration messagesConfig) {
        ...
        if(args.length <= 2){
            msgManager.sendMessage(sender,messagesConfig.getString("commandAddTimeError"),true);
            return;
        }
        ...
        playerDataManager.addPlayTime(player,time);

        msgManager.sendMessage(sender,messagesConfig.getString("commandAddTimeCorrect")
